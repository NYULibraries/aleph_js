...
function bs_pds_login() {
	if (getCookie('_aleph_pds_passive_login') == null) {
		jQuery("#holdings table#items td.links a:first").each(function(index, a) {
			jQuery.get(this.href, function(data, textStatus, xmlHttpRequest) {
				var pds_url_matches = pds_url_regex().exec(data);
				if (pds_url_matches != null) {
					var pds_url = pds_url_matches[1] + encodeURIComponent(window.location);
					setCookie('_aleph_pds_passive_login', 1);
